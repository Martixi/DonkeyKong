...
void
PrepareGame(Sdl &sdl, GameObjects &objects, GameEntity &player, Colors &colors,
			GameEntity &enemy, GameEntity *barrels,GameEntity &portal);
void GenerateEntity(GameEntity *player, SDL_Renderer *renderer, const char *bmp,
					int width, int height, int startingX,int startingY);
